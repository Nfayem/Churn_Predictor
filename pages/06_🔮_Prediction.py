...
if st.session_state.get("authentication_status"):
    ...
    def models():
        ...
        xgb_model = joblib.load('./models/XB_model.joblib')
        lr_model = joblib.load('./models/LR_model.joblib')
        svc_model = joblib.load('./models/SVC_model.joblib')
        knn_model = joblib.load('./models/KNN_model.joblib')
        fnn_model = joblib.load('./models/FNN_model.joblib')
        return rf_model, gb_model, xgb_model, lr_model, svc_model, knn_model, fnn_model

    RF, GB, XB, LR, SVC, KNN, FNN = models()
    ...
    # Select model 
    selected_model = st.selectbox('Select a Model', ['', 'Random Forest', 'GBoost', 'XGBoost', 'Logistic', 'Support Vector', 'K Nearest', 'Feedforward'], 
                                    key='selected_model',
                                    index=0)
